...
import { builtInNormalizer, mergeAppError, runNormalizerPipeline } from '../normalizer'
...
describe('runNormalizerPipeline', () => {
  ...
  it('custom normalizer returning partial merges with built-in (later keys win)', () => {
    const customNormalizer: Normalizer = (raw, _current) => ({ code: 'CUSTOM', message: 'custom message', raw })
    const err = new TypeError('failed')
    const result = runNormalizerPipeline(err, [customNormalizer], builtInNormalizer)
  ...
  it('custom normalizer returning null → accumulator unchanged', () => {
    ...
    const err = new TypeError('failed')
    const result = runNormalizerPipeline(err, [nullNormalizer], builtInNormalizer)
    expect(result.code).toBe('NETWORK_ERROR')
  })

  it('custom normalizer throwing → onError called, pipeline continues', () => {
    const onError = vi.fn()
    const throwingNormalizer: Normalizer = () => { throw new Error('boom') }
    const err = new TypeError('failed')
    const result = runNormalizerPipeline(err, [throwingNormalizer], builtInNormalizer, onError)
  ...
  it('result missing code → assigned GRACEFULERRORS_UNKNOWN', () => {
    const weirdNormalizer: Normalizer = (raw) => ({ code: '' as string, raw }) as AppError
...
describe('mergeAppError', () => {
  it('undefined values never overwrite existing values', () => {
    ...
    const partial: AppError = { code: 'FOO', status: undefined, message: undefined, raw: 'r' }
  ...
  it('context is shallow-merged', () => {
    const current: AppError = { code: 'FOO', context: { field: 'email', extra: 'keep' } }
    const partial: AppError = { code: 'FOO', context: { field: 'name', newKey: 'added' } }
    const result = mergeAppError(current, partial)
