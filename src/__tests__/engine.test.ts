...
type Code = 'NOT_FOUND' | 'UNAUTHORIZED' | 'RATE_LIMITED' | 'SERVER_ERROR' | 'INLINE_ERR'
...
describe('Normalizer precedence', () => {
  ...
  it('normalizers[] alone: custom normalizer result merged before built-in', () => {
    const custom: Normalizer<Code> = () => ({ code: 'NOT_FOUND', message: 'custom msg' })
    const engine = makeEngine({ normalizers: [custom] })
  ...
  it('both normalizer + normalizers: normalizer wins, dev warning emitted', () => {
    ...
    const engine = makeEngine({
      normalizer: custom,
      normalizers: [shouldNotRun as unknown as Normalizer<Code>],
    })
    engine.handle('raw')
...
describe('State integration', () => {
  ...
  it('onDropped fires on queue_overflow', () => {
    ...
    const engine = makeEngine({ maxConcurrent: 1, maxQueue: 0, dedupeWindow: 0, onDropped })
...
describe('Renderer', () => {
  ...
  it('modal: onDismiss callback provided → calling it releases the slot', () => {
    ...
    const renderCall = (renderer.render as ReturnType<typeof vi.fn>).mock.calls[0]
  ...
  it('toast: onDismiss NOT provided in lifecycle', () => {
    ...
    const engine = makeEngine({}, renderer)
    engine.handle(structuredNotFound)
    const lifecycle = (renderer.render as ReturnType<typeof vi.fn>).mock.calls[0][1]
...
describe('Hook safety', () => {
  ...
  it('throwing onNormalized does not interrupt pipeline', () => {
    ...
    const engine = makeEngine({
      onNormalized: () => { throw new Error('boom') },
      onRouted,
    })
    engine.handle(structuredNotFound)
...
describe('Dev mode', () => {
  it('debug: true → console.log trace emitted in dev/test env', () => {
    ...
    expect(logSpy).toHaveBeenCalledWith('[gracefulerrors trace]', expect.any(Object))
  ...
  it('both normalizer + normalizers → warn in development', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('normalizer takes precedence'))
