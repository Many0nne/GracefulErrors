...
describe("Registry match", () => {
  it("error with registry entry → returns entry.ui", () => {
    ...
    expect(router.route(error, registry, baseConfigFor("NOT_FOUND"))).toBe(
      "toast",
    );
  });
...
describe("requireRegistry", () => {
  it("requireRegistry: true + no entry → throws", () => {
    ...
    const error: AppError<Code> = { code: "NETWORK_ERROR" };
    expect(() =>
      router.route(error, registry, { ...baseConfig, requireRegistry: true }),
    ).toThrow(
...
describe("routingStrategy context", () => {
  it("activeCount and queueLength are passed correctly to the strategy", () => {
    ...
    expect(strategy).toHaveBeenCalledWith(error, registry["NOT_FOUND"], {
      activeCount: 2,
      queueLength: 1,
    });
  });
...
describe("Edge cases", () => {
  it("routingStrategy throws → treated as returning null (continue to registry lookup)", () => {
    const router = createUIRouter<Code>();
    const error: AppError<Code> = { code: "NOT_FOUND" };
    const strategy: RoutingStrategy<Code> = () => {
