...
describe('createHttpPreset', () => {
  it('returns all 11 HTTP codes by default', () => {
    ...
    const codes = ['HTTP_400', 'HTTP_401', 'HTTP_403', 'HTTP_404', 'HTTP_409', 'HTTP_422', 'HTTP_429', 'HTTP_500', 'HTTP_502', 'HTTP_503', 'HTTP_504']
  ...
  it('HTTP_401 defaults to modal with dismissible: false', () => {
    ...
    expect((entry as any).uiOptions?.dismissible).toBe(false)
  })
  ...
  it('HTTP_500 defaults to toast error', () => {
    ...
    expect((entry as any).uiOptions?.severity).toBe('error')
  })
  ...
  it('HTTP_400 defaults to toast warning', () => {
    ...
    expect(entry.ui).toBe('toast')
    expect((entry as any).uiOptions?.severity).toBe('warning')
  })

  it('overrides a specific code', () => {
    const preset = createHttpPreset({
      HTTP_401: { ui: 'toast', message: 'Session expired. Please log in again.' },
    })
    const entry = preset['HTTP_401']!
    expect(entry.ui).toBe('toast')
  ...
  it('preserves non-overridden codes when overrides are provided', () => {
    const preset = createHttpPreset({ HTTP_401: { ui: 'toast', message: 'Custom' } })
  ...
  it('does not mutate the defaults on successive calls', () => {
    const first = createHttpPreset({ HTTP_400: { ui: 'modal', message: 'Changed' } })
  ...
  it('returns a plain registry usable with spread in createErrorEngine config', () => {
    ...
    const registry = {
      ...preset,
      PAYMENT_FAILED: { ui: 'modal' as const, message: 'Payment could not be processed.' },
    }
