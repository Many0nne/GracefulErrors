...
function makeEngine(overrides?: Partial<ErrorEngine>): ErrorEngine {
  return {
    handle: vi.fn().mockReturnValue({ handled: true, error: { code: 'ERR' }, uiAction: 'toast' }),
    clear: vi.fn(),
    clearAll: vi.fn(),
...
describe('provideErrorEngine + useErrorEngine', () => {
  ...
  it('no provider — useErrorEngine returns null and logs in dev', () => {
    ...
    let captured: ErrorEngine | null = undefined as unknown as ErrorEngine | null
    ...
    expect(spy).toHaveBeenCalledWith(expect.stringContaining('[gracefulerrors]'))
  ...
  function makeFieldConsumer(field: string) {
    return defineComponent({
      setup() {
        ...
        return () => h('div', { 'data-testid': 'error' }, error.value ? error.value.code : 'none')
      },
    })
  }

  function wrapWithProvider(engine: ErrorEngine, child: ReturnType<typeof defineComponent>) {
    return defineComponent({
      setup() {
        provideErrorEngine(engine)
        return () => h(child)
  ...
  it('captured error is forwarded to engine.handle', async () => {
    ...
    const Parent = defineComponent({
      setup() {
        provideErrorEngine(engine)
        return () =>
          h(ErrorBoundaryWithEngine, { fallback: 'fallback' }, {
            default: () => h(ThrowingChild, { shouldThrow: true }),
          })
      },
    })
