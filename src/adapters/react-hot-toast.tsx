  ...
  function render<TCode extends string = string>(intent: RenderIntent<TCode>, lifecycle: { onDismiss?: () => void }): void {
    switch (intent.ui) {
      case 'toast': {
        ...
        let id: string
        if (severity === 'error') {
          id = toast.error(message, { icon: opts.icon, duration: opts.duration ?? 4000 })
        } else if (severity === 'success') {
          id = toast.success(message, { icon: opts.icon, duration: opts.duration ?? 4000 })
        } else {
          // warning / info → generic toast with custom icon
          const defaultIcon = severity === 'warning' ? '⚠️' : 'ℹ️'
          id = toast(message, { icon: opts.icon ?? defaultIcon, duration: opts.duration ?? 4000 })
        }
